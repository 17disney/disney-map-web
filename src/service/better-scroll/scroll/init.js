...
const DEFAULT_OPTIONS = {
  ...
  observeDOM: true,
  autoBlur: true,
  initScrollWidth: false,

  /**
   * for Horizontal wheel
   * horWheel: {
   *   tabListPosition: []
   * }
   */
...
export function initMixin(BScroll) {
  BScroll.prototype._init = function(el, options) {
    ...
    // init private custom events 初始化私有自定义事件
    this._events = {}
    ...
    if (this.options.horWheel) {
      this._initHorWheel()
    }

    if (this.options.initScrollWidth) {
      this._initScrollWidth()
    }
  ...
  BScroll.prototype._initScrollWidth = function() {
    let { wheelWrapperClass = 'scroll-content' } = this.options.horWheel

    const wheelWrapper = this.wrapper.querySelector('.' + wheelWrapperClass)
    const items = wheelWrapper.children
    this.items = items
    ...
    if (!this.options.horWheel) {
      wheelWrapper.style.width = `${width}px`
    }
  }

  // scroll 宽度初始化
  BScroll.prototype._initHorWheel = function() {
    this._initScrollWidth()
    let wrapperRect = getRect(this.wrapper)

    let itemsWidth = this.itemsWidth
    let itemsPos = []
    // 集合元素焦点位置
    let posStart = 0
    for (let i = 0; i < itemsWidth.length; i++) {
      ...
      if (i === 0) {
        ...
        pos = (itemsWidth[i - 1] + itemsWidth[i]) / 2
        itemsPos[i] = pos + posStart
      }
      posStart += pos
    }
    ...
    let width = this.scrollerWidth
    width = width + wrapperRect.width - itemsWidth[itemsWidth.length - 1]

    let { wheelWrapperClass = 'scroll-content' } = this.options.horWheel
    const wheelWrapper = this.wrapper.querySelector('.' + wheelWrapperClass)
    wheelWrapper.style.width = `${width}px`
    wheelWrapper.style.marginLeft = `${wrapperRect.width / 2 - itemsWidth[0] / 2}px`
  }
  ...
  BScroll.prototype.refresh = function() {
    ...
    const {wheel, horWheel} = this.options
    if (wheel) {
