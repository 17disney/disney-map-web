...
export function coreMixin(BScroll) {
  ...
  BScroll.prototype._move = function (e) {
    ...
    if (this.options.probeType > 1) {
      ...
      if (this.options.horWheel) {
        this.selectedIndex = this._getHorWheelIndex(this.x)
      }
      this.trigger('scroll', msg)
    }
  ...
  BScroll.prototype._end = function (e) {
    ...
    if (this.options.momentum && duration < this.options.momentumLimitTime && (absDistY > this.options.momentumLimitDistance || absDistX > this.options.momentumLimitDistance)) {
      ...
      if (this.options.horWheel) {
        let index = this._getHorWheelIndex(newX)
        newX = -this.itemsPos[index]
      }
      ...
      if (this.options.horWheel) {
        let index = this._getHorWheelIndex(newX)
        newX = - this.itemsPos[index]
        time = this.options.horWheel.adjustTime || 400
      }
    }
  ...
  BScroll.prototype._checkClick = function (e) {
    ...
    if (!this.moved) {
      if (this.options.wheel) {
        ...
        return true
      } else if (this.options.horWheel) {

        let _offset = Math.round(this.pointX - this.wrapperWidth / 2)
        let newX = this.x - _offset
        let index = this._getHorWheelIndex(-newX)
        newX = - this.itemsPos[index]
        this.scrollTo(newX, 0, 400, ease.swipe)
        return true
      } else {
        if (!preventClick) {
          if (this.options.tap) {
  ...
  BScroll.prototype.scrollTo = function (x, y, time = 0, easing = ease.bounce) {
    ...
    if (!time || this.options.useTransition) {
      ...
      if (this.options.wheel) {
        ...
        this.selectedIndex = this._getHorWheelIndex(x)
      }
    } else {
