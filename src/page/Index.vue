  ...
  <div>
    <ds-header :wheel="true" :line="false">
      <ds-tab @change="change" :wheel="true" v-model="type">
        <ds-tab-wheel-item v-if="item.showTab" :key="item.id" :id="item.id" v-for="item in ATT_TYPE">{{item.name}}</ds-tab-wheel-item>
      </ds-tab>
      <div slot="focus-icon">
    ...
    <div v-show="mode == 'list'">
      <div class="att-list-wrapper">
        <att-list>
          <att-list-item v-for="(item, index) in activeList" :data="item" :wait="waits[item.aid]" :key="index"></att-list-item>
        </att-list>
      </div>
